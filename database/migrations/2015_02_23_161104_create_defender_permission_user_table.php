...
class CreateDefenderPermissionUserTable extends Migration
{
    ...
    public function up()
    {
        Schema::create(config('defender.permission_user_table', 'permission_user'), function (Blueprint $table) {
            $table->unsignedBigInteger(config('defender.user_key', 'owner_id'));
            $table->foreign(config('defender.user_key', 'owner_id'))->references('id')
                ->on(config('defender.user_table', 'owners'))
                ->onDelete('cascade');
